...
//            BufferedImage hendImage = ImageIO.read(hendImgFile);
     ...
     * @param qrUrl 二维码识别地址
     */
    public static boolean createQRCode(File codeFile,
                                       int width,
                                       int height,
                                       String qrUrl) {
        ...
        if (!codeFile.exists()) {
            try {
                final boolean mkdirs = codeFile.getParentFile()
                                               .mkdirs();
                System.out.println("mkdirs = " + mkdirs);
                codeFile.createNewFile();
            } catch (IOException e) {
                log.error("createQRCode:createNewFile failed", e);
                return success;
            }
        }
        ...
        try {
            MultiFormatWriter multiFormatWriter = new MultiFormatWriter();
            BitMatrix         bitMatrix         =
                    multiFormatWriter.encode(qrUrl, BarcodeFormat.QR_CODE, width, height, hints);
            BufferedImage     image             = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
            for (int x = 0; x < width; x++) {
                for (int y = 0; y < height; y++) {
            ...
            log.error("Create QR Code Exception", e);
        }
        return success;
    }
