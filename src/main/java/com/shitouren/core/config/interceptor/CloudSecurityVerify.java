...
@Slf4j public class CloudSecurityVerify {
    ...
    public static boolean signVerify(CloudRedisTemplate cloudRedisTemplate, HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        try {
            ...
            if(o instanceof UserLoginWx){
                userLoginDto = (UserLoginWx) o;
            }
            if (userLoginDto == null || StringUtil.isEmpty(String.valueOf(userLoginDto.getUserId()))) {
                throw new CloudException(ExceptionConstant.登录失效);
            }

            final Integer userId = userLoginDto.getUserId();
            final String  tokenToVerify     = cloudRedisTemplate.get(userId + "1");
            //log.info("userId = {}, token = {}, tokenToVerify = {}", userId, token, tokenToVerify);
            if (!StringUtils.equals(tokenToVerify, token)) {//反向校验token
                throw new CloudException(ExceptionConstant.登录失效);
            }
            WebContextHolder.set(userId);
...
//            }
            ...
            log.error("", e);
            throw new CloudException(ExceptionConstant.请求失败);
        } catch (Exception e) {
